...
async function meFetch<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
        ...
        console.error(
            `[magiceden] API error ${res.status} for ${init?.method || "GET"} ${path}:`,
            text,
        );
        if (res.status === 401) {
            throw new Error(
...
export async function getListNFTTx(params: {
    ...
    // Magic Eden listing uses the GET /instructions/sell endpoint (price in SOL).
    const query = new URLSearchParams({
        seller: params.seller,
        tokenMint: params.tokenMint,
        tokenAccount: params.tokenAccount,
        price: params.priceSol.toString(),
        tokenSize: (params.tokenSize ?? 1).toString(),

...
async function predictionFetch<T>(
  ...
  const res = await fetch(`${PREDICTION_BASE_URL}${path}`, {
    ...init,
    headers,
  });
...
export async function listEvents(params?: {
...
}): Promise<{ data: PredictionEvent[]; pagination: { total: number; hasNext: boolean } }> {
  ...
  if (params?.end !== undefined) searchParams.set("end", params.end.toString());
...
export async function getPositions(
  ownerPubkey: string
): Promise<{ data: PredictionPosition[]; pagination: { total: number; hasNext: boolean } }> {
...
export async function claimPayout(
  ...
  return predictionFetch(`/positions/${encodeURIComponent(positionPubkey)}/claim`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ownerPubkey }),
  });

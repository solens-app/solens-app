...
import { getSOLBalance, getTokenAccounts, getWalletOverview } from "@/app/lib/solana";
import { getTokenPrices, resolveTokenMint, getSwapOrder, getDecimals } from "@/app/lib/jupiter";
import {
  searchPools,
  getPoolDetails,
...
function rawUnitsToUiAmountString(rawAmount: string | bigint, decimals: number): string {
...
function findTokenHolding(holdings: TokenHolding[], token: string): TokenHolding | null {
  ...
  return (
    ...
    holdings.find((holding) => holding.mint.toUpperCase().startsWith(upper)) ||
    null
  );
}
...
- To place a bet: use buy_prediction with the exact marketId string, side (YES or NO), and amount in USD (minimum $1). Users need USDC to trade.
...
const tools: OpenAI.Chat.Completions.ChatCompletionTool[] = [
      ...
      parameters: {
        ...
        properties: {
          tokens: {
            type: "array",
            items: { type: "string" },
            description: "Array of token symbols to get prices for (e.g. ['SOL', 'USDC', 'BONK'])",
          },
        },
        required: ["tokens"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          token: {
            type: "string",
            description: "Token symbol or mint address to transfer (e.g. USDC, BONK, JUP).",
          },
          amount: {
            type: "number",
            description: "Amount of token to transfer in human-readable units. Omit only when sendAll is true.",
          },
          sendAll: {
            type: "boolean",
            description: "Set to true when the user asks to send the full token balance.",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          inputToken: {
            type: "string",
            description: "The token symbol to swap FROM (e.g. 'SOL', 'USDC', 'BONK')",
          },
          outputToken: {
            type: "string",
            description: "The token symbol to swap TO (e.g. 'USDC', 'SOL', 'JUP')",
          },
          amount: {
            type: "number",
            description: "The amount of the input token to swap (in human-readable units, e.g. 0.1 for 0.1 SOL)",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Search query - typically the token pair like 'SOL-USDC', 'SOL USDC', or a pool address",
          },
        },
        required: ["query"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          poolAddress: {
            type: "string",
            description: "The Meteora pool address (from search_meteora_pools results)",
          },
          depositToken: {
            type: "string",
            description: "The symbol of the token the user wants to deposit (e.g. 'USDC', 'SOL')",
          },
          depositAmount: {
            type: "number",
            description: "Amount of the deposit token in human-readable units (e.g. 0.5 for 0.5 USDC)",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          positionAddress: {
            type: "string",
            description: "The position address (from get_user_positions results)",
          },
          percentage: {
            type: "number",
            description: "Percentage of liquidity to remove (1-100). Defaults to 100.",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          symbol: {
            type: "string",
            description: "Token symbol/ticker (e.g. 'DOGE', 'PEPE')",
          },
          description: {
            type: "string",
          ...
          initialBuySOL: {
            type: "number",
            description: "Amount of SOL for initial buy (optional, default 0). E.g. 0.1 for 0.1 SOL initial purchase.",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          virtualPoolAddress: {
            type: "string",
            description: "The virtual pool address (from get_claimable_fees results)",
          },
          programId: {
            type: "string",
            description: "The fee share program ID (from get_claimable_fees results)",
          },
          isCustomFeeVault: {
            type: "boolean",
            description: "Whether this uses a custom fee vault (from get_claimable_fees results)",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Search keyword (e.g. 'bitcoin', 'NBA', 'election'). If empty, browses by category.",
          },
          category: {
            type: "string",
            description: "Category filter: crypto, sports, politics, esports, culture, economics, tech. Used when browsing without a query.",
          },
          filter: {
            type: "string",
            description: "Status filter: 'new' (last 24h), 'live' (in progress), 'trending'. Optional.",
          },
        },
        required: [],
      },
  ...
  },
  {
    ...
    function: {
      name: "buy_prediction",
      description:
        "Places a buy order on a prediction market. Buys YES or NO contracts on an event outcome. Returns a transaction for the user to sign. Users need USDC to trade. Amount is in USD (e.g. 5.0 = $5). Minimum $5. IMPORTANT: You must use the exact marketId from search results (e.g. 'POLY-1928733-0'), NOT the event title or number.",
      parameters: {
        type: "object",
        properties: {
          marketId: {
            type: "string",
            description: "The exact marketId string from search_prediction_events results (e.g. 'POLY-1928733-0'). Must be the real ID, not a title or number.",
          },
          side: {
            type: "string",
            enum: ["YES", "NO"],
          ...
          amountUsd: {
            type: "number",
            description: "Amount to spend in USD (e.g. 5.0 for $5.00). Minimum $5.00.",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          positionPubkey: {
            type: "string",
            description: "The position's public key (from get_prediction_positions results)",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          positionPubkey: {
            type: "string",
            description: "The position's public key (from get_prediction_positions results, must have claimable=true)",
          },
        },
        required: ["positionPubkey"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          symbol: {
            type: "string",
            description: "The collection symbol (e.g. 'okay_bears')",
          },
          limit: {
            type: "number",
            description: "Number of listings to return (default 10, max 20)",
          },
        },
        required: ["symbol"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          tokenMint: {
            type: "string",
            description: "The NFT's mint address (from listing data)",
          },
          seller: {
            type: "string",
            description: "The seller's wallet address (from listing data)",
          },
          tokenATA: {
            type: "string",
            description:
          ...
          price: {
            type: "number",
            description: "The listing price in SOL (priceRaw from listing data)",
          },
          sellerExpiry: {
            type: "number",
            description:
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          tokenMint: {
            type: "string",
            description: "The NFT's mint address (from get_my_nfts results)",
          },
          priceSOL: {
            type: "number",
            description: "The listing price in SOL (e.g. 1.5 for 1.5 SOL)",
          },
        },
...
function generateQuickReplies(
  ...
  for (const { name, result } of results) {
    ...
    if (name === "search_meteora_pools" && Array.isArray(data.pools)) {
      const pools = data.pools as { name: string; address: string; apr: string }[];
    ...
    if (name === "search_prediction_events" && Array.isArray(data.events)) {
      const events = data.events as {
        title: string;
        markets: { marketId: string; title: string; yesPrice: string; noPrice: string }[];
      }[];
    ...
    if (name === "get_prediction_positions" && Array.isArray(data.positions)) {
      const positions = data.positions as {
        positionPubkey: string;
        market: string;
        side: string;
    ...
    if (name === "get_claimable_fees" && Array.isArray(data.positions)) {
      const positions = data.positions as { tokenMint: string; claimableSOL: string }[];
      for (const pos of positions.slice(0, 5)) {
        replies.push({
    ...
    if (name === "get_nft_listings" && Array.isArray(data.listings)) {
      ...
      for (const l of listings.slice(0, 5)) {
        const label = l.name.length > 20 ? l.name.slice(0, 17) + "..." : l.name;
        replies.push({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const txSigner =
      typeof txSignerWalletAddress === "string" && txSignerWalletAddress.trim().length > 0
    ...
    const transferToolConfirmPhrase =
      chatChannel === "telegram"
        ? telegramSigningMode === "privy_server" || telegramSigningMode === "custodial"
    ...
    console.log("[chat] walletAddress:", walletAddress, "| txSigner:", txSigner, "| message:", message);
    ...
    while (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
      ...
      for (const toolCall of toolCalls) {
        ...
        if (!walletAddress) {
          result = { error: "No wallet connected. Please log in first." };
        } else {
          switch (toolCall.function.name) {
            ...
            case "get_wallet_overview":
              result = await getWalletOverview(walletAddress);
              break;
            ...
            case "get_token_price": {
              ...
              if (mints.length === 0) {
                result = { error: "Could not resolve any of the provided token symbols." };
              } else {
                const prices = await getTokenPrices(mints);
                // Map back to symbols for readability
                const priceMap: Record<string, string> = {};
                for (let i = 0; i < tokens.length; i++) {
                  const mint = await resolveTokenMint(tokens[i]);
                  const priceInfo = prices.find((p) => p.mint === mint);
                  if (priceInfo) {
                    priceMap[tokens[i].toUpperCase()] = `$${priceInfo.price}`;
                  }
                }
            ...
            case "transfer_sol": {
              ...
              try {
                if (toAddress === spendWallet) {
                  result = { error: "Sender and recipient addresses must be different." };
                  break;
                }
                if (sendAll !== true && (amountSOL === undefined || amountSOL === null)) {
                  result = { error: "amountSOL is required unless sendAll is true." };
                  break;
                }
                if (
                  sendAll !== true &&
                  (!Number.isFinite(amountSOL) || (amountSOL as number) <= 0)
                ) {
                  result = { error: "amountSOL must be a positive number." };
                  break;
                }

                const balance = await getSOLBalance(spendWallet);
                const rentReserveLamports = await getSystemAccountRentReserveLamports();
                const totalReserveLamports =
                  rentReserveLamports + SOL_TRANSFER_FEE_BUFFER_LAMPORTS;
                const maxSendableLamports = balance.lamports - totalReserveLamports;
                if (maxSendableLamports <= 0) {
                  ...
                  if (txSigner && walletAddress && txSigner !== walletAddress) {
                    const linkedBal = await getSOLBalance(walletAddress);
                ...
                const desiredLamports =
                  sendAll === true
                    ? maxSendableLamports
                    : Math.round((amountSOL as number) * LAMPORTS_PER_SOL);
                if (desiredLamports <= 0) {
                  result = { error: "Amount is too small after converting to lamports." };
                  break;
                }
                ...
                const actualAmountSol = lamportsToSend / LAMPORTS_PER_SOL;
                ...
                const msg =
                  error instanceof Error ? error.message : "Failed to prepare SOL transfer";
                result = { error: msg };
              }
              break;
            }
            ...
            case "transfer_token": {
              const args = JSON.parse(toolCall.function.arguments);
              const { toAddress, token, amount, sendAll } = args as {
                toAddress: string;
                token: string;
                amount?: number;
                sendAll?: boolean;
              ...
              try {
                ...
                if (!token || typeof token !== "string") {
                  result = { error: "Token symbol or mint is required." };
                  break;
                }

                const tokenQuery = token.trim();
                const holdings = await getTokenAccounts(spendWallet);
                const heldMatch = findTokenHolding(holdings, tokenQuery);
                const mintAddress = isSolanaPublicKey(tokenQuery)
                  ? tokenQuery
                  : heldMatch?.mint ?? (await resolveTokenMint(tokenQuery));
                if (!mintAddress) {
                  result = { error: `Could not resolve token: ${tokenQuery}` };
                  break;
                }

                const tokenHolding =
                  holdings.find((t) => t.mint === mintAddress) ?? heldMatch;
                if (!tokenHolding) {
                ...
                if (!sendFullBalance && (amount === undefined || amount === null)) {
                  result = { error: "Token amount is required unless sendAll is true." };
                  break;
                }
                ...
                if (!sendFullBalance) {
                  try {
                    amountRaw = uiAmountToRawUnits(amount as number, tokenHolding.decimals);
                    amountLabel = rawUnitsToUiAmountString(amountRaw, tokenHolding.decimals);
                  } catch {
                    result = { error: "Invalid token amount." };
                    break;
                  }
                }
                ...
                try {
                  const balanceRaw = BigInt(tokenHolding.rawAmount);
                ...
                pendingAction = {
                  type: "transfer",
                  transaction: tx.transaction,
                  assetSymbol: tokenHolding.symbol || tokenQuery.toUpperCase(),
            ...
            case "initiate_swap": {
              ...
              try {
                ...
                if (!inputMint) {
                  result = { error: `Could not resolve token: ${inputToken}` };
                  break;
                }

                const outputMint = await resolveTokenMint(outputToken);
                if (!outputMint) {
                  result = { error: `Could not resolve token: ${outputToken}` };
                  break;
                }

                const decimals = getDecimals(inputMint);
                const order = await getSwapOrder(inputMint, outputMint, amount, decimals, spendWallet);
                ...
                result = {
                  success: true,
                  message: `Swap order created: ${amount} ${inputToken} for ${outputToken}. ${outputInfo} Transaction ready for user to sign and confirm.`,
                  requestId: order.requestId,
                  estimatedOutput: order.estimatedOutput,
                };
              } catch (error) {
                const msg = error instanceof Error ? error.message : "Swap order failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "search_meteora_pools": {
              ...
              try {
                ...
                if (pools.length === 0) {
                  result = { message: "No pools found for that search query." };
                } else {
                  result = {
                    pools: pools.map((p) => ({
                      address: p.address,
                ...
                const msg = error instanceof Error ? error.message : "Pool search failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "get_user_positions": {
              try {
                ...
                if (positions.length === 0) {
                  result = { message: "You have no positions in this pool." };
                } else {
                  // Enrich positions with pool details
                  const enriched = [];
                  const poolCache = new Map<string, Awaited<ReturnType<typeof getPoolDetails>>>();
                  for (const pos of positions) {
                    if (!poolCache.has(pos.poolAddress)) {
                      poolCache.set(pos.poolAddress, await getPoolDetails(pos.poolAddress));
                    }
                    const pool = poolCache.get(pos.poolAddress);
                    // Format raw amounts using token decimals
                    const xDecimals = pool?.tokenXDecimals ?? 6;
                    const yDecimals = pool?.tokenYDecimals ?? 9;
                    const xAmount = Number(pos.totalXAmount) / 10 ** xDecimals;
                    const yAmount = Number(pos.totalYAmount) / 10 ** yDecimals;
                    ...
                    enriched.push({
                      ...
                      tokenXAmount: xAmount.toFixed(Math.min(xDecimals, 6)),
                      tokenYAmount: yAmount.toFixed(Math.min(yDecimals, 6)),
                      currentPrice: pool?.currentPrice ?? 0,
                      tvl: pool ? `$${pool.tvl.toLocaleString()}` : "?",
                    });
                  }
            ...
            case "add_liquidity": {
              const args = JSON.parse(toolCall.function.arguments);
              const { poolAddress, depositToken, depositAmount } = args;
              ...
              try {
                ...
                const depositUpper = (depositToken as string).toUpperCase();
                const isTokenX = pool.tokenXSymbol.toUpperCase() === depositUpper;
                const isTokenY = pool.tokenYSymbol.toUpperCase() === depositUpper;

                if (!isTokenX && !isTokenY) {
                  result = { error: `Token ${depositToken} is not in this pool. This pool has ${pool.tokenXSymbol} and ${pool.tokenYSymbol}.` };
                  break;
                }
            ...
            case "remove_liquidity": {
              const args = JSON.parse(toolCall.function.arguments);
              const { poolAddress, positionAddress, percentage } = args;
              const bps = Math.min(Math.max(Math.round((percentage ?? 100) * 100), 100), 10000);
            ...
            case "launch_token": {
              const args = JSON.parse(toolCall.function.arguments);
              const { name, symbol, description, imageUrl, twitter, website, telegram, initialBuySOL } = args;

              try {
                // Check SOL balance — launching requires ~0.1 SOL for rent + fees
                const minSOLForLaunch = 0.1 + (initialBuySOL || 0);
                ...
                console.log("[bags] Token info created:", tokenInfo.tokenMint);
                ...
                console.log("[bags] Fee config created, meteoraConfigKey:", feeConfig.meteoraConfigKey);
                ...
                const initialBuyLamports = Math.round((initialBuySOL || 0) * 1e9);

                const feeConfigTxStrings = feeConfig.transactions.map((t) => t.transaction);
                ...
                const msg = error instanceof Error ? error.message : "Token launch failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "get_claimable_fees": {
              try {
                const positions = await getClaimablePositions(walletAddress);
                if (positions.length === 0) {
                  result = { message: "You have no claimable fees from Bags token launches." };
                } else {
                  result = {
                    positions: positions.map((p) => ({
                      tokenMint: p.baseMint,
                      claimableSOL: (p.totalClaimableLamportsUserShare / 1e9).toFixed(4),
                      claimableDisplay: p.claimableDisplayAmount,
                      virtualPool: p.virtualPool || p.virtualPoolAddress,
                ...
                const msg = error instanceof Error ? error.message : "Failed to fetch claimable fees";
                result = { error: msg };
              }
              break;
            }
            ...
            case "claim_fees": {
              const args = JSON.parse(toolCall.function.arguments);
              const { tokenMint, virtualPoolAddress, programId, isCustomFeeVault } = args;

              try {
                const claimResult = await createClaimTransactions({
                  feeClaimer: walletAddress,
                  tokenMint,
                  virtualPoolAddress,
                  claimVirtualPoolFees: true,
                  claimDammV2Fees: false,
                  isCustomFeeVault: isCustomFeeVault || false,
                  feeShareProgramId: programId,
                });

                const txStrings = claimResult.transactions.map((t) => t.transaction);
                ...
                const msg = error instanceof Error ? error.message : "Claim fees failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "search_prediction_events": {
              ...
              try {
                let events;
                if (query && query.trim()) {
                  const searchResult = await searchPredictionEvents(query, 10);
                  events = searchResult.data;
                } else {
                  const listResult = await listPredictionEvents({
                ...
                if (!events || events.length === 0) {
                  result = { message: "No prediction events found for that search." };
                } else {
                  ...
                  const mapped = events.map((e) => ({
                    ...
                    volume: microUsdToDisplay(e.volumeUsd),
                    markets: (e.markets || [])
                      .filter((m) => m.status === "open" && (m.closeTime - nowSec) > MIN_TIME_LEFT)
                      .map((m) => ({
                        marketId: m.marketId, // USE THIS EXACT ID for buy_prediction
                        title: m.title || m.metadata?.title,
                        status: m.status,
                        yesPrice: m.pricing.buyYesPriceUsd
                          ? `$${(m.pricing.buyYesPriceUsd / 1_000_000).toFixed(2)}`
                          : "N/A",
                        noPrice: m.pricing.buyNoPriceUsd
                          ? `$${(m.pricing.buyNoPriceUsd / 1_000_000).toFixed(2)}`
                          : "N/A",
                        yesProbability: m.pricing.buyYesPriceUsd
                          ? `${((m.pricing.buyYesPriceUsd / 1_000_000) * 100).toFixed(0)}%`
                          : "N/A",
                        noProbability: m.pricing.buyNoPriceUsd
                  ...
                  if (mapped.length === 0) {
                    result = { message: "No open prediction markets found. All matching markets are closed or expired." };
                  } else {
                    result = {
                      events: mapped,
                ...
                const msg = error instanceof Error ? error.message : "Event search failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "buy_prediction": {
              const args = JSON.parse(toolCall.function.arguments);
              const { marketId, side, amountUsd } = args;

              try {
                if (amountUsd < 5) {
                  result = { error: "Minimum bet amount is $5.00" };
                  break;
                }

                console.log("[prediction] Creating order:", { marketId, side, amountUsd });

                // Get market details for display
                const market = await getPredictionMarket(marketId);
                console.log("[prediction] Market status:", market.status, "title:", market.metadata?.title || market.title);
                if (market.status !== "open") {
                  result = { error: `Market is ${market.status}, not open for trading.` };
                  break;
                }
                ...
                console.log("[prediction] Order created:", {
                  ...
                  fee: orderResponse.order.estimatedTotalFeeUsd,
                });
                ...
                pendingAction = {
                  ...
                  contracts: orderResponse.order.contracts,
                  orderPubkey: orderResponse.order.orderPubkey,
                  marketTitle: market.metadata?.title || market.title,
                };

                const price = side.toUpperCase() === "YES"
                  ? market.pricing.buyYesPriceUsd
                  : market.pricing.buyNoPriceUsd;
                const priceDisplay = price ? `$${(price / 1_000_000).toFixed(2)}` : "market price";

                result = {
                  success: true,
                  message: `Order prepared: Buy ${orderResponse.order.contracts} ${side.toUpperCase()} contracts on "${market.metadata.title}" for $${amountUsd.toFixed(2)} at ${priceDisplay}/contract. Please confirm the transaction.`,
                  contracts: orderResponse.order.contracts,
                  estimatedFee: microUsdToDisplay(orderResponse.order.estimatedTotalFeeUsd),
                };
              } catch (error) {
                const msg = error instanceof Error ? error.message : "Buy prediction failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "get_prediction_positions": {
              try {
                const positionsResult = await getPredictionPositions(walletAddress);
                const positions = positionsResult.data;

                if (!positions || positions.length === 0) {
                  result = { message: "You have no prediction market positions." };
                } else {
                  result = {
                    positions: positions.map((p) => ({
                      ...
                      currentValue: p.valueUsd ? `$${microUsdToDisplay(p.valueUsd)}` : "N/A",
                      pnl: p.pnlUsd ? `$${microUsdToDisplay(p.pnlUsd)}` : "N/A",
                      pnlPercent: p.pnlUsdPercent !== null ? `${p.pnlUsdPercent.toFixed(1)}%` : "N/A",
                      payout: `$${microUsdToDisplay(p.payoutUsd)}`,
                      claimable: p.claimable,
                      claimed: p.claimed,
                      marketStatus: p.marketMetadata.status,
                      marketResult: p.marketMetadata.result || "pending",
                    })),
                  };
                }
              } catch (error) {
                const msg = error instanceof Error ? error.message : "Failed to fetch prediction positions";
                result = { error: msg };
              }
              break;
            }
            ...
            case "sell_prediction": {
              ...
              try {
                const closeResponse = await closePredictionPosition(positionPubkey, walletAddress);

                // Get position info for display
                const positionsResult = await getPredictionPositions(walletAddress);
                const position = positionsResult.data?.find((p) => p.pubkey === positionPubkey);
                const marketTitle = position?.marketMetadata.title || "Unknown Market";
                ...
                const msg = error instanceof Error ? error.message : "Sell prediction failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "claim_prediction": {
              ...
              try {
                const claimResponse = await claimPredictionPayout(positionPubkey, walletAddress);

                const payoutDisplay = microUsdToDisplay(claimResponse.position.payoutAmountUsd);
                ...
                const msg = error instanceof Error ? error.message : "Claim payout failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "search_nft_collection": {
              const args = JSON.parse(toolCall.function.arguments);
              try {
                const stats = await getMECollectionStats(args.symbol);
            ...
            case "get_nft_listings": {
              ...
              try {
                const listings = await getMEListings(args.symbol, 0, limit);
                if (listings.length === 0) {
                  ...
                  result = {
                    listings: listings.map((l) => ({
                      ...
                      name: l.token?.name || l.tokenMint.slice(0, 8) + "...",
                      seller: l.seller,
                      ...
                      collection: l.token?.collectionName || args.symbol,
                    })),
                    _note:
                      "Use the EXACT tokenMint, seller, tokenAddress (as tokenATA), priceRaw (as price), expiry, and auctionHouse values when calling buy_nft.",
                  };
                }
              } catch (error) {
                const msg =
            ...
            case "get_my_nfts": {
              try {
                const nfts = await getMEWalletNFTs(walletAddress);
                if (nfts.length === 0) {
                  result = { message: "You don't own any NFTs." };
                } else {
                  result = {
                    nfts: nfts.map((n) => ({
                      ...
                      name: n.name,
                      collection:
                        n.collectionName || n.collection || "Unknown",
                      image: n.image || null,
                      listStatus: n.listStatus || "unlisted",
                      price: n.price ? `${n.price} SOL` : null,
                    })),
                  };
                }
              } catch (error) {
            ...
            case "buy_nft": {
              const args = JSON.parse(toolCall.function.arguments);
              try {
                console.log("[magiceden] Creating buy tx:", args);
                const tx = await getMEBuyNowTx({
                  ...
                  auctionHouseAddress: args.auctionHouseAddress,
                });

                let nftName = args.tokenMint.slice(0, 8) + "...";
                try {
                  const nftInfo = await getMENFTByMint(args.tokenMint);
                ...
                const msg =
                  error instanceof Error ? error.message : "Buy NFT failed";
                result = { error: msg };
              }
              break;
            }
            ...
            case "list_nft": {
              const args = JSON.parse(toolCall.function.arguments);
              try {
                const priceLamports = Math.round(args.priceSOL * 1e9);
                ...
                const msg =
                  error instanceof Error ? error.message : "List NFT failed";
                result = { error: msg };
              }
              break;
            }
    ...
    const assistantMessage = choice.message.content || "I'm not sure how to help with that.";

...
async function signAll(
    ...
    for (const tx of txs) {
        ...
        signed.push(
            await signWithAppWallet({ walletId, transactionBase64: base64 }),
        );
    }
    return signed;
}
...
export async function signAndBroadcastAction(params: {
    ...
    try {
        if (type === "swap") {
            if (!action.requestId)
                return { ok: false, error: "swap action missing requestId" };
            ...
            if (status === "Pending")
                return {
                    ...
                    pending: true,
                    signature: sig,
                    error: (r as { warning?: string }).warning,
                };
            return {
                ok: false,
                error:
        ...
        return {
            ok: false,
            error:
                (r as { error?: string }).error || status || "Broadcast failed",
        };
    } catch (e) {

...
export async function POST(request: NextRequest) {
  ...
  if (!body.action || !body.action.type) {
    return NextResponse.json({ error: "action with type is required" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: exec.error }, { status: 400 });
  } catch (e) {
    const detail = e instanceof Error ? e.message : String(e);
    console.error("[sign-broadcast] failed", { privyUserId, detail });
    return NextResponse.json({ error: "Sign-broadcast failed", detail }, { status: 500 });
  }
}
